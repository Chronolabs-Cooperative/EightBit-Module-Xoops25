...
$xoopsOption['template_main'] = 'db:eightbit_artist.html';
include_once dirname(dirname(__DIR__)) . DS . 'header.php';
include_once dirname(dirname(__DIR__)) . DS . 'class' . DS . 'pagenav.php';

if (!$artist = xoops_getModuleHandler('artists')->getByKey($_REQUEST['key']))
{
    redirect_header(XOOPS_URL . '/modules/' . basename(__DIR__) . '/index.php', 7, 'No Artist Found for Key!');
    exit(0);
}
...
foreach(xoops_getModuleHandler('artists', basename(__DIR__))->getCrumbs(xoops_getModuleHandler('alpha')->get($artist->getVar('alphaid'))->getVar('charley')) as $chars => $values)
{
...
$GLOBALS['xoopsTpl']->assign('artist', array(   'artist'     =>      $artist->getVar('artist'),
    'url'       =>      XOOPS_URL . '/modules/' . basename(__DIR__) . '/artist.php?key='.md5($artist->getVar('id')),
    'albums'   =>      $artist->getVar('albums'),
    'tracks'    =>      $artist->getVar('tracks'),
    'bytes'     =>      $artist->getVar('bytes'),
    'hits'      =>      $artist->getVar('hits'),
    'playtime'  =>      eightbit_secondsDiplay($artist->getVar('totalseconds'))));


$artistsids = array();
$artistsids[$artist->getVar('id')] = $artist->getVar('id');
