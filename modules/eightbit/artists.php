...
$xoopsOption['template_main'] = 'db:eightbit_artists.html';
...
foreach(xoops_getModuleHandler('artists', basename(__DIR__))->getCrumbs($_REQUEST['artistalpha']) as $chars => $values)
{
    $title[$chars] = $chars;
    $breadcrumb[$chars]['chars'] = $chars;
    $breadcrumb[$chars]['url'] = XOOPS_URL . '/modules/' . basename(__DIR__) . "/artists.php?artistalpha=" . $chars;
}
...
$xoopsOption['xoops_pagetitle'] = "Artists: " .implode (" -> ", $title);

$totalseconds = 0;
$alpha = array();
if (strlen($_REQUEST['artistalpha'])<3)
{
    foreach(xoops_getModuleHandler('artists', basename(__DIR__))->selAlpha($_REQUEST['artistalpha']) as $chars => $values)
    {
...
$ids = xoops_getModuleHandler('artists', basename(__DIR__))->getIDsAlpha($_REQUEST['artistalpha']);

$artistsids = array();
$criteria = new Criteria('alphaid', '(' . implode(', ', array_keys($ids)) . ')', 'IN');
foreach(xoops_getModuleHandler('alpha_artists', basename(__DIR__))->getObjects($criteria, true) as $key => $object)
...
$pagenav = new XoopsPageNav($ttl, (isset($_REQUEST['artistlimit'])?$_REQUEST['artistlimit']:$GLOBALS['limit']), (isset($_REQUEST['artiststart'])?$_REQUEST['artiststart']:$GLOBALS['start']), 'artiststart', 'artistalpha='.$_REQUEST['artistalpha'].'&artistlimit='.(isset($_REQUEST['artistlimit'])?$_REQUEST['artistlimit']:$GLOBALS['limit']).'&'.http_build_query(eightbit_RemoveFieldKeywords('artist', parse_str($_SERVER['QUERY_STRING']))));
